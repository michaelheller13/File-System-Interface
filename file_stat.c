...
int main(int argc, char *argv[]) {
  ...
  if (fstat(fd, &file_info) == 0) {
    ...
    gmode = (((1 << 3) - 1) & (mode >> (6 - 1))); // TO-DO
     
    // omode comes from the low 3 bits in mode
    omode = mode & ((1<<3)-1); // TO-DO
    ...
    char bufg[1024];
    struct group *result;
    getgrgid_r(getgid(), &grp, bufg, sizeof(bufg), &result);

    printf(" owner user name = %s\n", pwbuf.pw_name); // TO-DO: man getpwuid 
    printf(" owner group name = %s\n", grp.gr_name); // TO-DO: man getgrgid 
